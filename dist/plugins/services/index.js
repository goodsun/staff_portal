...
function run(cmd) {
    return new Promise(resolve => {
        (0, child_process_1.exec)(cmd, { timeout: 5000 }, (error, stdout, stderr) => {
...
// GET: ログ表示
router.get('/log', auth_1.requireAuth, async (req, res) => {
    const id = req.query.id;
    const svc = SERVICES.find(s => s.id === id);
    if (!svc?.logFile || !fs_1.default.existsSync(svc.logFile)) {
