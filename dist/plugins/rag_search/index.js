...
function httpGet(urlStr) {
    return new Promise((resolve, reject) => {
        const req = http_1.default.get(urlStr, res => {
            let d = '';
            res.on('data', c => d += c);
            res.on('end', () => {
                try {
...
router.post('/', auth_1.requireAuth, async (req, res) => {
    ...
    if (!query)
        return res.redirect(url('/rag') + `?source=${source}`);
    ...
    try {
        ...
        if (source === 'local') {
            ...
            const stdout = await new Promise((resolve, reject) => {
                (0, child_process_1.execFile)('node', [HQ_SCRIPT, query, `--collection=${collection}`, `--n=${n}`], {
                    timeout: 30000,
