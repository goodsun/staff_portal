...
const GEN_SCRIPT = path_1.default.join(__dirname, 'gen.js');
...
if (!fs_1.default.existsSync(SCENE_DIR))
    fs_1.default.mkdirSync(SCENE_DIR, { recursive: true });
function loadPresets() {
    try {
        const touch = JSON.parse(fs_1.default.readFileSync(path_1.default.join(IMAGE_GEN_DATA, 'touch_presets.json'), 'utf-8'));
        const model = JSON.parse(fs_1.default.readFileSync(path_1.default.join(IMAGE_GEN_DATA, 'model_presets.json'), 'utf-8'));
        return { touch, model };
    }
    catch {
        return { touch: { presets: [], default: '' }, model: { presets: [], default: '' } };
    }
}
...
`;
fs_1.default.writeFileSync(GEN_SCRIPT, GEN_SCRIPT_CONTENT);
// ── キャストデータ読み込み ────────────────────────
function loadCasts() {
    ...
    return fs_1.default.readdirSync(CASTS_DIR)
        ...
        .map(d => {
        try {
            const p = JSON.parse(fs_1.default.readFileSync(path_1.default.join(CASTS_DIR, d, 'profile.json'), 'utf-8'));
...
router.get('/api/scenes', auth_1.requireAuth, (_req, res) => {
    const files = fs_1.default.existsSync(SCENE_DIR)
        ? fs_1.default.readdirSync(SCENE_DIR).filter(f => IMG_EXTS.includes(path_1.default.extname(f).toLowerCase()))
        : [];
    res.json(files.map(f => ({ filename: f, url: url('/image_gen/scene/' + f) })));
});
// ── 背景画像配信 ─────────────────────────────────
router.get('/scene/:file', auth_1.requireAuth, (req, res) => {
    const filePath = path_1.default.join(SCENE_DIR, path_1.default.basename(req.params.file));
...
const sceneUploader = (0, upload_1.makeAssetUploader)(SCENE_DIR);
router.post('/upload/scene', auth_1.requireAuth, sceneUploader.single('file'), (req, res) => {
    if (!req.file)
        return res.status(400).json({ error: 'no file' });
    res.json({ ok: true, filename: req.file.filename, url: url('/image_gen/scene/' + req.file.filename) });
});
// ── 背景画像削除 ──────────────────────────────────
router.delete('/scene/:file', auth_1.requireAuth, (req, res) => {
    const filePath = path_1.default.join(SCENE_DIR, path_1.default.basename(req.params.file));
    if (!fs_1.default.existsSync(filePath))
...
// ── API: プリセット（タッチ・モデル） ────────────
router.get('/api/presets', auth_1.requireAuth, (_req, res) => {
    res.json(loadPresets());
});
// ── API: キャストデータJSON ───────────────────────
router.get('/api/casts', auth_1.requireAuth, (_req, res) => {
    const casts = loadCasts().map(c => {
        const stylesResolved = {};
        for (const [sk, sv] of Object.entries(c.styles)) {
            const imgFile = sv.image;
            const imgUrl = imgFile && fs_1.default.existsSync(path_1.default.join(CASTS_DIR, c.id, imgFile))
                ? url('/image_gen/cast-avatar/' + c.id + '/' + imgFile) : '';
            stylesResolved[sk] = { ...sv, imageUrl: imgUrl };
        }
        const mainImg = c.avatars?.main || '';
        const mainImgUrl = mainImg && fs_1.default.existsSync(path_1.default.join(CASTS_DIR, c.id, mainImg))
...
// ── API: キャストアバター画像 ─────────────────────
router.get('/cast-avatar/:id/:file', auth_1.requireAuth, (req, res) => {
    const filePath = path_1.default.join(CASTS_DIR, req.params.id, req.params.file);
    if (!filePath.startsWith(CASTS_DIR) || !fs_1.default.existsSync(filePath))
        return res.status(404).end();
    res.sendFile(filePath);
});
// ── GET: フォーム ─────────────────────────────────
router.get('/', auth_1.requireAuth, (_req, res) => {
    ...
    const castOptions = casts.map(c => `<option value="${c.id}">${c.emoji} ${c.name}（${c.role}）</option>`).join('');
    const body = `
    <div class="header">
...
function resolveGenArgs(body) {
    ...
    if (castId) {
        try {
            const profile = JSON.parse(fs_1.default.readFileSync(path_1.default.join(CASTS_DIR, castId, 'profile.json'), 'utf-8'));
            ...
            if (imgFile) {
                const candidate = path_1.default.join(CASTS_DIR, castId, imgFile);
                if (fs_1.default.existsSync(candidate))
                    refImagePath = candidate;
            }
        }
        catch { }
    }
    ...
    if (castRefsRaw) {
        try {
            const castRefsInput = JSON.parse(castRefsRaw);
            for (const cr of castRefsInput) {
                if (!cr.id)
                    continue;
                try {
                    const profile = JSON.parse(fs_1.default.readFileSync(path_1.default.join(CASTS_DIR, cr.id, 'profile.json'), 'utf-8'));
                    const styleData = profile.styles?.[cr.style] || profile.styles?.[profile.default_style] || {};
                    const imgFile = styleData.image || '';
                    if (imgFile) {
                        const candidate = path_1.default.join(CASTS_DIR, cr.id, imgFile);
                        if (fs_1.default.existsSync(candidate))
                            refsArr.push({ path: candidate, label: cr.label });
                    }
                }
                catch { }
            }
    ...
    if (bgFile) {
        const bgPath = path_1.default.join(SCENE_DIR, path_1.default.basename(bgFile));
        if (fs_1.default.existsSync(bgPath))
            refsArr.push({ path: bgPath, label: 'BG', type: 'background' });
    }
...
router.post('/api/generate', auth_1.requireAuth, (req, res) => {
    const resolved = resolveGenArgs(req.body);
    if (!resolved)
        return res.status(400).json({ ok: false, error: 'prompt required or GEMINI_API_KEY missing' });
    const { prompt, args, filename, outPath } = resolved;
    console.log('[image_gen] api/generate prompt:', prompt.slice(0, 80));
    (0, child_process_1.execFile)('node', args, { timeout: 90000, env: { ...process.env, HOME: '/home/node' } }, (err, _stdout, stderr) => {
        if (err || !fs_1.default.existsSync(outPath)) {
...
router.post('/', auth_1.requireAuth, (req, res) => {
    const resolved = resolveGenArgs(req.body);
    if (!resolved)
        return res.redirect(url('/image_gen'));
    const { prompt, args, filename, outPath } = resolved;
...
// ── client.js 配信 ───────────────────────────────
router.get('/client.js', auth_1.requireAuth, (req, res) => {
...
// ── 画像配信 ─────────────────────────────────────
router.get('/img/:filename', auth_1.requireAuth, (req, res) => {
    const filePath = path_1.default.join(OUT_DIR, path_1.default.basename(req.params.filename));
    if (!fs_1.default.existsSync(filePath))
