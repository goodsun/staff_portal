...
// ── HTTP ヘルパー ─────────────────────────────────
function ragGet(path) {
    return new Promise((resolve, reject) => {
        const full = RAG_URL + path;
        const lib = full.startsWith('https') ? https_1.default : http_1.default;
...
async function fetchUrl(targetUrl) {
    return new Promise((resolve, reject) => {
        const lib = targetUrl.startsWith('https') ? https_1.default : http_1.default;
...
router.get('/', auth_1.requireAuth, async (_req, res) => {
    ...
    const deleted = _req.query?.deleted;
...
// ── GET: ドキュメント編集フォーム ─────────────────
router.get('/edit', auth_1.requireAuth, async (req, res) => {
    const docId = req.query.id ?? '';
    const collection = req.query.collection ?? 'default';
    if (!docId)
        return res.redirect(url('/rag_admin'));
    try {
...
router.post('/update', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            throw new Error('メタデータのJSON形式が正しくありません');
        }
...
// ── POST: ドキュメント削除 ────────────────────────
router.post('/delete-document', auth_1.requireAuth, async (req, res) => {
    const docId = (req.body.doc_id ?? '').trim();
    const collection = (req.body.collection ?? 'default').trim();
    if (!docId)
        return res.redirect(url('/rag_admin'));
...
// ── GET: 投入フォーム ─────────────────────────────
router.get('/ingest', auth_1.requireAuth, async (_req, res) => {
    let collections = [];
    try {
        const data = await ragGet('/collections');
        collections = (data.collections ?? []).map((c) => c.name);
    }
    catch { }
...
// ── POST: URL投入 ────────────────────────────────
router.post('/ingest-url', auth_1.requireAuth, async (req, res) => {
    ...
    if (!ingestUrl)
        return res.redirect(url('/rag_admin/ingest'));
    try {
...
// ── POST: テキスト直接投入 ───────────────────────
router.post('/ingest-text', auth_1.requireAuth, async (req, res) => {
    const text = (req.body.text ?? '').trim();
    ...
    if (!text)
        return res.redirect(url('/rag_admin/ingest'));
    try {
        const metadata = { ingested_at: new Date().toISOString() };
        if (title)
...
// ── POST: コレクション削除 ───────────────────────
router.post('/delete-collection', auth_1.requireAuth, async (req, res) => {
